...
const server = http.createServer(function(request, response) {
    ...
    if (request.method == 'POST') {
        ...
        request.on('end', function() {
            ...
            console.log(post);
            const result = rental.price(Number(post.age), Number(post.licence), Number(post.tier), parseBool(post.accidentCause), parseBool(post.season))
            console.log(result);
            ...
            <html>
                <body>
                    <form id="calcForm" method="post" action="http://localhost:3000">Driver: <br>
                        ...
                        <input type="number" name="tier" />
                        <label for="html">Class of the car (1 to 5)</label><br>
                        <input type=checkbox name="accidentCause" />
                        <label for="html">Have you caused accidentCauseidents?</label><br>
