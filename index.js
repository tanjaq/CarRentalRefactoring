...
const server = http.createServer(function(request, response) {
    ...
    if (request.method == 'POST') {
        ...
        request.on('end', function() {
            ...
            console.log(post);
            const result = rental.calculateRentalPrice(Number(post.age), Number(post.licenceYears), Number(post.carClass), parseBool(post.hasAccident), parseBool(post.hasParticipatedAccident), parseBool(post.isHighSeason))
            console.log(result);
            ...
            <html>
                <body>
                    <form id="calcForm" method="post" action="http://localhost:3000">Driver: <br>
                        ...
                        <input type=checkbox name="hasParticipatedAccident" />
